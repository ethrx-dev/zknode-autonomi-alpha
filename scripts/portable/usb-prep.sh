...
# Resolve image names from compose (arch-suffixed refs already set in .env).
IMAGES_FILE="$DST/images.list"
: > "$IMAGES_FILE"
{
    grep -ho 'image: *[^ ]*' docker-compose.yml docker-compose.zymkey.yml 2>/dev/null \
        | sed 's/^image: *//; s/["'"'"']//g' \
        | sed "s/\${IMAGE_MIXNET}/zeros\/mixnet-node:${IMG_ARCH}/; s/\${IMAGE_MIXNET_PROXY}/zeros\/mixnet-proxy:${IMG_ARCH}/; s/\${IMAGE_ANTD}/zeros\/antd:${IMG_ARCH}/; s/\${IMAGE_DASHBOARD}/zknode-dashboard:latest/; s/\${IMAGE_WALLETSHIELD}/walletshield-rebuild:latest/" \
        | sort -u
} > "$IMAGES_FILE"

step "Vendoring docker images for $IMG_ARCH..."
mkdir -p "$DST/images"
found=0
while IFS= read -r img; do
    ...
    if docker image inspect "$img" >/dev/null 2>&1; then
        ...
        step "  save $img -> images/$fname"
        docker save "$img" | gzip > "$DST/images/$fname"
        found=1
    else
        warn "  image NOT found locally, skipping: $img"
    fi
done < "$IMAGES_FILE"
[ "$found" = "1" ] || warn "no images were vendored — target will need docker pull/build"
