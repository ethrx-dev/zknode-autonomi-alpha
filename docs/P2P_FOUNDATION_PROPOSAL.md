...
The program is built on two hardware tracks — **DIY SCM4 Compute Module** for makers and **SEN400 Beta Node** for plug-and-play deployment — and leverages a formal collaboration between **0KN** (mixnet privacy layer) and **Autonomi** (autonomous data storage network).
...
**0KN** and **Autonomi** are collaborating to integrate:

| 0KN Provides | Autonomi Provides |
|--------------|-------------------|
...
```
User App → WalletShield → Mixnet (0KN) → Courier → Autonomi Storage
...
- **0KN Core Team** — Protocol development, security audits, hardware validation
