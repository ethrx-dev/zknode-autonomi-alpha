...
| IP | `192.168.9.118` (static on `192.168.9.0/24`) |
...
- **User**: `zero-tech` (primary ops user)
- **Passwords**: `r00ts3cur3` (sudo)
- **SSH key**: `~/.ssh/id_ed25519_zk` (on build machine, `~/.ssh/id_ed25519_zk.pub` on SCM4)
- **Hostname**: `zknode` (via `/etc/hosts` on LAN: `192.168.9.118 zknode.local`)

---

## 3. Docker Containers

All 15 containers use `network_mode: host` (mixnet + dashboard + proxy). Managed via `docker compose` in `/home/zero-tech/zknode-autonomi/`.
...
- Docker compose: `/home/zero-tech/zknode-autonomi/docker-compose.yml`
- Dashboard server: `/home/zero-tech/zknode-autonomi/zknode-dashboard/server/index.js`
- Dashboard HTML: `/home/zero-tech/zknode-autonomi/zknode-dashboard/public/index.html`
- Wiki data: `/home/zero-tech/wikis/p2p-infrastructure/wiki/`
  - 497 wiki pages (P2P Foundation archive)
  - Managed by llm-wiki MCP tools on port 18765
- NomadNet data: `/home/zero-tech/nomadnet-new/` → mounted at `/var/nomadnet/` in containers
...
5. **Add wiki page count** — reads `/home/zero-tech/wikis/p2p-infrastructure/wiki/` directory listing
...
# Verify
iw dev wlan0 info
# → Interface wlan0, type ibss, ssid "zknode-mesh", channel 1 (2412 MHz)
```
...
5. **MetaMask RPC URL**: `http://192.168.9.118:9292`
...
**File**: `/home/zero-tech/katzenpost/cmd/chatd/main.go` (line 366)
...
- **Host zkchat**: `/home/zero-tech/zknode01/bin/zkchat`
...
- **Binary**: `/home/zero-tech/.local/share/ant/bin/ant-node-0.13.0` (statically linked ARM64, 14MB)
- **CLI**: `/home/zero-tech/.local/bin/ant` (v0.2.8, 39MB)
- **Daemon**: Running as systemd --user service, port 33899
- **Node**: PID 110371, port 12000 QUIC, DHT connecting
- **Data**: `/home/zero-tech/ant-node-data/node-1/`
...
```
Build Machine (blaqbox)         SCM4 (zknode.local)
192.168.9.17                    192.168.9.118
       │                              │
       │  SSH (id_ed25519_zk)         │
       │  Dashboard: http://:8080     │
       │  MetaMask: http://:9292     │
       │  WiFi mesh: zknode-mesh ch1  │
       │  TCP mesh: :4242             │
       ▼                              ▼
┌─────────────────┐      ┌─────────────────────────────┐
